...
# CSS with PROPERLY FIXED CODE BLOCKS
st.markdown("""
    <style>
    .main {
        padding: 2rem;
    ...
    /* FIX SIDEBAR TEXT - MAKE IT LIGHT/WHITE */
    ...
    /* COMPLETELY FIX CODE BLOCKS - FORCE WHITE TEXT */
    .stCodeBlock {
    ...
    /* Force Streamlit code component text to be white */
...
st.markdown("<div style='text-align: center; color: #666; font-size: 1.1em; margin-bottom: 30px;'>Quick 4-question check before launching your campaign</div>", unsafe_allow_html=True)
...
# Progress indicator
progress = min(st.session_state.step / 4, 1.0)
st.progress(progress)
st.markdown(f"<div style='text-align: center; color: #666; margin-bottom: 20px;'>Step {st.session_state.step} of 4</div>", unsafe_allow_html=True)
...
# STEP 4: Final Check and Get Templates
elif st.session_state.step == 4:
    st.markdown('<div class="step-header">⚡ Quick final check</div>', unsafe_allow_html=True)
    ...
    if st.session_state.answers.get('data_collection', {}).get('photos', False):
        ...
        photo_consent = "N/A"
        st.info("✅ No additional questions needed. Click below to get your templates!")
